...
CUTOFF_LEN=8192                   # 与 new.sh 保持一致
TEMPLATE=qwen                     # 与 new.sh 保持一致
TARGET_MODEL=Qwen2.5-7B-Instruct             # 与 new.sh 保持一致
TOTAL_BATCH_SIZE=128
PER_DEVICE_BATCH_SIZE=1
GRAD_ACCUM_STEPS=$((TOTAL_BATCH_SIZE / (PER_DEVICE_BATCH_SIZE * GPUS_PER_NODE * N_NODE)))
LEARNING_RATE=2.0e-5
OUTPUT_DIR=/fs-computility/llmit_d/shared/zhuhe/html_project/trained_model/saves/${TARGET_MODEL}/full/sft-${DATA_SETTING}/${TARGET_MODEL}_CUTOFF_LEN${CUTOFF_LEN}_LR${LEARNING_RATE}_N_NODE${N_NODE}_GBS${TOTAL_BATCH_SIZE}

echo "总批次大小: ${TOTAL_BATCH_SIZE}"
echo "梯度累积步数: ${GRAD_ACCUM_STEPS}"

TRAIN_FULL_DIR="/fs-computility/llmit_d/shared/zhuhe/html_project/trained_model"
