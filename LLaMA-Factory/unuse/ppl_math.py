...
def create_yaml_config(target_model, template, info, packing, gpus_per_node, data_size):
    """创建训练配置yaml文件"""
    ...
    # 计算每0.5个epoch的步数
    # 实际的训练步数 = 数据量 / (每个设备批次大小 * GPU数量 * 梯度累积步数)
    actual_steps_per_epoch = data_size // (PER_DEVICE_BATCH_SIZE * gpus_per_node * GRAD_ACCUM_STEPS)
    save_steps = max(1, actual_steps_per_epoch // 2)  # 每0.5个epoch保存
    
    print(f"Data size: {data_size}, Steps per epoch: {actual_steps_per_epoch}, Save every {save_steps} steps")
...
def sft(target_model, template, data_path, info, mode="sft", packing=False, gpus_per_node=4):
    ...
    yaml_file = create_yaml_config(target_model, template, info, packing_str, gpus_per_node, len(data))
